...
export class EditCustomerComponent implements OnInit {
  ...
  constructor(
    ...
    this.eventService.events$.pipe(filter(({ key }: EventData) =>
      ...
      .subscribe(({ key, data }: EventData) => {
        ...
        this.customerApi.findById(data.value)
          .subscribe((ret:Customers)=>{
            this.listOrder = ret.orders;
            this.name = ret.name;
            this.phone = ret.phone;
            this.address = ret.address;
            this.sumPrice = ret.sumPrice;
        })
      })
  }
  closeModal() {
  ...
  update() {
    ...
    this.customerApi.patchAttributes(this.currentId, data).subscribe((ret) => {
      this.closeModal();
